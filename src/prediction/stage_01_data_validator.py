...
import pandas as pd
from src.utility import read_params, create_directory_path, values_from_schema_function,\
    get_logger_object_of_prediction
import argparse
import shutil
...
class DataValidator:
    def __init__(self, config, logger, is_logging_enable=True):
        try:
            ...
            self.config = config
            self.file_path = self.config['data_source']['Prediction_Batch_Files']
            self.good_file_path = self.config['artifacts']['prediction_data']['good_file_path']
            self.bad_file_path = self.config['artifacts']['prediction_data']['bad_file_path']
            self.archive_bad_file_path = self.config['artifacts']['prediction_data']['archive_bad_file_path']
            self.prediction_schema_file = self.config['config']['schema_prediction']
    ...
    def value_from_schema(self):
        ...
        """
        try:
            return values_from_schema_function(self.prediction_schema_file)
        except Exception as e:
            generic_exception = GenericException(
                "Error occurred in module [{0}] class [{1}] method [{2}]"
                    .format(self.__module__, DataValidator.__name__,
        ...
        try:
            ...
            for file in files:
                ...
                if df.shape[1] != no_of_column:
                    destination_file_path = os.path.join(self.bad_file_path, file)
                    self.logger.log(f"file: {file} has incorrect number of column hence moving file to bad file path {destination_file_path}")
                    shutil.move(file_path, destination_file_path)
        except Exception as e:
            generic_exception = GenericException(
                "Error occurred in module [{0}] class [{1}] method [{2}]"
...
def validation_main(config_path: str, datasource: str, is_logging_enable=True,execution_id=None,executed_by=None) -> None:
    try:
