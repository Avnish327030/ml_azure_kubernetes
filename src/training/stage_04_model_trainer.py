...
class ModelTrainer:
    ...
    def data_preparation(self):
        try:
            ...
            input_features=preprocessing.scale_data(data=input_features,path=self.scaler_path,is_dataframe_format_required=True,
            )
    ...
    def begin_training(self):
        try:
            ...
            with mlflow.start_run():
                ...
                x_train, x_val, x_test, y_train, y_val, y_test=self.split_dataset(input_features=input_feature,target_features=target_feature)
                ...
                self.generate_score_and_plot_graph(model=best_model,x=x_train,y=y_train,title="training")

                #validation accuracy
                self.generate_score_and_plot_graph(model=best_model,x=x_val,y=y_val,title="Validation")

                #Testing accuracy
                self.generate_score_and_plot_graph(model=best_model,x=x_test,y=y_test,title="Testing")
